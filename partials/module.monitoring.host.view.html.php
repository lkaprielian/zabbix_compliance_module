...
$table->setHeader([
	...
	make_sorting_header(_('Compliance'), 'name', $data['sort'], $data['sortorder'], $view_url)
...
function addGroupRow($data, &$rows, $group_name, $parent_group_name, $level, &$child_stat) {
	...
	foreach ($group['hosts'] as $hostid) {
		...
		if ($total_problem_count == 0) {
			...
			$problems_link = (new CSpan('OK'));
			$problems_link->addClass(ZBX_STYLE_GREEN);
		}
		else {
		...
		$table_row_host = new CRow([
			$col1,
			// (new CCol(getHostInterface($interface)))->addClass(ZBX_STYLE_NOWRAP),// delete interface
			// getHostAvailabilityTable($host['interfaces']), // delete availability
			$host['tags'],
			$problems_link
			// ($host['status'] == HOST_STATUS_MONITORED) //delete status
			// 	? (new CSpan(_('Enabled')))->addClass(ZBX_STYLE_GREEN)
			// 	: (new CSpan(_('Disabled')))->addClass(ZBX_STYLE_RED),
			// [
			// 	$data['allowed_ui_latest_data'] // delete latest data
			...
			// $host['dashboards'] // delete dashboards
			// 	? [
	...
	$table_row = new CRow([
		$col2,
		$group_problems_div, // see if i add them or not
		(new CCol())
			-> setColSpan(2) // here is number of problems number of col
	]);
